...
class JiraFetcher:
    ...
    def __init__(self):
        ...
        if not url or not token:
            raise ValueError("Missing required Jira environment variables (JIRA_URL, JIRA_API_TOKEN)")

        self.config = JiraConfig(url=url, username=username or "", api_token=token)
        self.jira = self._create_client(verify_ssl)
        self.preprocessor = TextPreprocessor(self.config.url)

    def _create_client(self, verify_ssl: bool) -> Jira:
        """Create Jira client with appropriate auth for cloud or on-premise."""
        if self.config.is_cloud:
            # Cloud: requires username + API token as password
            if not self.config.username:
                raise ValueError("JIRA_USERNAME is required for Atlassian Cloud")
            return Jira(
                url=self.config.url,
