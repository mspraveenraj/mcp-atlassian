...
class ConfluenceFetcher:
    ...
    def __init__(self):
        ...
        if not url or not token:
            raise ValueError("Missing required Confluence environment variables (CONFLUENCE_URL, CONFLUENCE_API_TOKEN)")

        self.config = ConfluenceConfig(url=url, username=username or "", api_token=token)
        self.confluence = self._create_client(verify_ssl)
        self.preprocessor = TextPreprocessor(self.config.url, self.confluence)

    def _create_client(self, verify_ssl: bool) -> Confluence:
        """Create Confluence client with appropriate auth for cloud or on-premise."""
        if self.config.is_cloud:
            # Cloud: requires username + API token as password
            if not self.config.username:
                raise ValueError("CONFLUENCE_USERNAME is required for Atlassian Cloud")
            return Confluence(
                url=self.config.url,
            ...
            # On-premise/Data Center: use Personal Access Token (PAT)
            return Confluence(
                url=self.config.url,
